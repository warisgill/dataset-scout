...
| M5 | 🔄 in progress | **Real-brief validation + edge hardening.** Each unfamiliar brief surfaces 2–4 ergonomic edges; we close them in the same session. Closed so far: `decompose --out` file/dir routing, decomposition-keyword precision (10× usable strategies on a frontier brief, 0 off-topic noise), `take: all` materialization-time blowup (25min → 62s end-to-end via auto-cap + `--max-rows-per-component`), **search round-robin fairness** (every direction contributes — was: a high-recall first direction starved every later direction, 1 of 6 directions represented; now: 6 of 6, 3× more useful strategies). Open: HF `load_dataset(streaming=True)` per-component setup overhead. |
